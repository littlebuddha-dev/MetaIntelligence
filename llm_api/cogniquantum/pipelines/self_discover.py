# /llm_api/cogniquantum/pipelines/self_discover.py
# Title: Self-Discover Pipeline
# Role: Implements the SELF-DISCOVER paper's methodology of dynamically selecting and executing reasoning strategies.
...
class SelfDiscoverPipeline:
    ...
    async def execute(
        ...
        try:
            # 1. 問題クラスをLLMで分類
            problem_class = await self._classify_problem(current_prompt)

            # 2. 最適な戦略を取得 (なければ動的生成)
            strategy = self.strategy_hub.get_best_strategy(problem_class)
            if not strategy:
                logger.info(f"問題クラス '{problem_class}' の既存戦略が見つかりません。動的に生成します。")
                strategy = await self._discover_strategy(current_prompt, problem_class)
    ...
    async def _classify_problem(self, prompt: str) -> str:
        """LLMを使って問題のクラスを分類する"""
        classify_prompt = f"""
        ...
        """
        response = await self.provider.call(classify_prompt, "", **self.base_model_kwargs)
        classification = response.get('text', 'general').strip().lower()
    ...
    async def _discover_strategy(self, prompt: str, problem_class: str) -> Strategy:
    ...
    async def _execute_strategy(self, strategy: Strategy, prompt: str, system_prompt: str) -> (Optional[str], Dict):
        """戦略の各ステップを順に実行する"""
        current_input = prompt
        thought_process = {"strategy_steps": []}
        ...
        for step_name in strategy.steps:
            ...
            response = await self.provider.call(step_prompt, system_prompt, **self.base_model_kwargs)
    ...
    def _format_response(self, solution: Optional[str], thought_process: Dict, v2_improvements: Dict) -> Dict[str, Any]:
        """統一されたレスポンス形式"""
